"""
Utility functions for RAG grounder runtime.
...
def find_evidence_spans(
    ...
    matched_ranges = []  # Global tracker: list of (start, end) tuples
    result = []
    ...
    exact_results = {}  # evidence_index -> (start, end, matched_text)
    ...
    fuzzy_results = {}  # evidence_index -> (start, end, matched_text, similarity)
    ...
    # Build result list (only include matched evidence strings)
    for idx, (evidence_clean, evidence_normalized) in enumerate(evidence_data):
        if idx in exact_results:
